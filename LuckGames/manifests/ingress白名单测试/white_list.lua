...
local redis_key_cache_prefix = "whitelist_cache:"  -- Redis 缓存前缀
local redis_cache_ttl = 30  -- Redis 缓存时间（秒）
...
local function is_ip_allowed(ip)
    ...
    local cache_key = redis_key_cache_prefix .. ip

    -- **Step 1: 先查缓存**
    local cached_result, err = red:get(cache_key)
    if cached_result and cached_result ~= ngx.null then
        ngx.log(ngx.INFO, "Cache hit for IP ", ip, ": ", cached_result)
        red:set_keepalive(10000, 100)
        return cached_result == "1"
    end

    -- **Step 2: 缓存未命中，查询 `sismember`**
    ...
    -- **Step 3: 查询结果写入 Redis 缓存**
    red:set(cache_key, res, "EX", redis_cache_ttl)
